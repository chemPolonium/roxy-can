...
**第二刀已落地（2026-09-04）**：`SampleCache` 内部改为**封存分块 + COW**——`chunks: Vec<Arc<Vec<点>>>`（只读、跨线程共享）+ `tail`（活跃追加区，512 点封存一次，封存即移动零拷贝）。`Clone` 因此从深拷贝变为"块引用计数 + 尾部拷贝"，发布代价从 O(全部点) 降到 O(指针数 + ≤512 点)。工作缓存的改动走 `Arc::make_mut`，绝不惊动 UI 仍在读的块。合并语义保持：直播采样走 O(增量) 追加快路；重叠/回退 backfill 仍走 O(总量) 的展平-合并-重分块（与旧扁平缓冲同价）。`range` 从返回切片改为返回知道自身长度的 `Samples` 迭代器（块级跳过 + 块内二分），`bucket_extremes` 泛化为 ExactSizeIterator 的单遍折叠。新增两条测试：发布视图在后续采样/修剪下冻结（COW 独立性）、跨封存块的合并与整块修剪。
