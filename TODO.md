...
- ~~**FR-3d --vector-probe 增强**~~ ✅（2026-09-16）：`--vector-probe --fibex <f>`——先校验描述文件并报告集群参数（bit/s 波特率、宏周期、静态槽、静载荷、33 项触发含首槽号），再按解析出的配置（不再用零配置探针）RX 试开 2 秒，首帧转储事件原始 64 字节（标注已知偏移）供**真机定位通道 A/B 的 channelMask 偏移**；文件损坏/非 FIBEX 整份拒绝（驱动调用前）。顺带修好 FIBEX 帧触发提取只认 `<FLEXRAY-FRAME-TRIGGERING>` 裸拼写的问题：CANoe 3.0 导出用 `fx:` 前缀 + `xsi:type` 属性，现在前缀/属性均容忍（demo 文件 33 项触发全数提出，测试锁定）。**剩余**：VN7640 插上跑 `--vector-probe --fibex <实际网络描述>` 拿真机诊断，确认收帧后把 A/B 偏移补进 `parse_fr_event`。
