...
动机：全应用单线程，总线活在 UI 的 60 fps tick 里——`MAX_TX_CATCHUP` 补发预算、激活锚定、Trace 揭示水位标都是这个耦合上的补丁；UI 一卡总线就停；真实硬件接入更是无从谈起。目标形态：总线核心按自己的时钟独立推进（1 ms 网格起步，硬件时间戳随后），前端按 60 fps 渲染，两者只通过**命令进、快照出**交流。
...
要做：从 App 拆出 `BusCore`（上面核心清单 + `step()` 方法），App 只留前端状态并持有 BusCore；总线逻辑全部改为 BusCore 方法。策略是先抄后删，每搬一块跑一次测试；app_tests 的构造与断言随之改走 BusCore。
验收：全部测试过，界面无可感知变化。
...
要做：BusCore 搬进独立线程，1 ms 绝对死线自驱推进仿真时钟；命令走 mpsc，快照用 Arc 换手（UI `try_lock`，拿不到沿用上一帧快照，永不阻塞）。发生器槽位在核心自己的 1 ms 网格上精确调度——`MAX_TX_CATCHUP` 预算与激活锚定大半自然消失，相关测试搬到核心的时钟语义上。
