...
- **S2 节点运行时**（接入核心线程）：
  - [ ] `ScriptNode` 实例：VM 状态 + 定时器表 + 事件注册表，一个节点绑定一条信道
  - [ ] 核心循环接入：帧到达按 (信道， ID) 分发、节点定时器并入核心死线
  - [ ] 主机内建补齐：`send` 帧、信号读写（走 DBC 编解码）、`now()`/`setTimer` 等
  - [ ] 每回调指令预算——坏脚本卡不死总线线程
  - [ ] Nodes 窗口：节点源码文本 + 编译错误列表 + 输出日志（交互全走文本）
  - [ ] 节点源码与绑定随工程持久化
