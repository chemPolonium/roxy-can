...
12. **State Tracker**（信号状态带视图，`src/ui/state.rs`）
   已落第一刀（2026-09-04）：对订阅采样历史做**状态分段**——值按 `%g` 六位有效数字量化，量化值不变的最长区段为一条状态带，带内居中标值；离散 0/1 出格子、缓变模拟量折叠成可读区段。窗口 = 左侧轨道管理（Bus/Message/Signal 三级下拉 + Track 按钮，自带选择器不进 Measurement Setup 注册表）+ 右侧标尺与状态带（名字与带同一张 draw list，对齐由构造保证）。恒走 live 贴边（回放中跟随 playhead），窗口宽 5 s–30 min 预设；删行经 `prune_signal`（已并入判断）自动退订。会话态：不随工程保存。
   要做：随工程持久化；接入 Measurement Setup 观察器注册表（PopupTarget + goto + 过滤弹窗）；右键把 Data/Graphics 信号直接加入跟踪；带内最小显示时长（抖动信号的一段碎带合并）；状态自定义（阈值区间命名如 Low/Mid/High）代替裸数值标签。
