pub fn extract_raw(data: &[u8; 8], start_bit: u64, size: u64, big_endian: bool) -> u64 {
    if size == 0 || size > 64 {
        return 0;
    }
    let mut raw: u64 = 0;
...
pub fn pack_raw(data: &mut [u8; 8], start_bit: u64, size: u64, big_endian: bool, raw: u64) {
    if size == 0 || size > 64 {
        return;
    }
    if !big_endian {
