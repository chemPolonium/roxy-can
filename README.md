...
- **Interactive Generator（节点中心）**：DBC 报文按数据库声明的周期发送（`GenMsgCycleTime` 优先于 `CycleTime`，事件触发不上定时器），按信号拖拽编辑物理值或按 hex 编辑；每个信号可挂 Ramp / Sine / Step / Random / Triangle / Counter 激励随仿真时间连续变化；**每个节点的生成器在 Network 窗口的节点详情里**（条目 + Add + 响应规则），总览窗口只留未分配条目与按 id 添加；**角色是总开关、条目开关是自定义**——切角色不改写逐条目开关，新建条目默认不发车，行内显示最近一次实际发射的载荷
...
- **Write 窗口**：整系统一条的滚动输出——节点 `print`（带归属）、装配层检查告警、熔断错误与命令事件分色分时戳；贴底跟随，一键清空，按类别过滤后可导出文本
- **Monitor 监控面板**：轻量纯文本行——自选信号/系统变量/派生信号一行一条（标签 + 实时值 + 单位），行内阈值着色规则让越限一眼可见；随工程保存
- **仿真节点**：类 CAPL 脚本语言（编译成字节码跑在自带 VM 上）驱动的自定义 ECU 节点——`on start` / `on message`（含 `*` 通配与错误帧事件）/ `on timer`（周期与一次性）事件驱动，读写 DBC 信号、收发帧、随机与波形内建；每回调 10 万指令预算，坏脚本卡不死总线；可**绑定**到 DBC 节点，绑定后发帧受该节点角色闸。编辑器侧栏带函数模板、处理器大纲、**静态收发事实表**（发送集按 DBC 归属标注、响应映射、系统变量访问集）与系统变量速插，源码带行号、语法着色与错误行标记。语言参考见 `docs/script_language.md`，可运行示例见 `examples/`
