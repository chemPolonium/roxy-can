...
- **Specification（规格监视 / ILO）**：View → Specification 打开，把 DBC 从"配置的来源"升级成"合同的来源"——实测到的流量与数据库声明的逐条对账，四类判定：**Unknown**（总线上出现数据库里没有的 ID）、**Dlc**（实际字节数与声明长度不符）、**Cycle**（相邻两帧的间隔偏离声明周期超过容差）、**Missing**（发过的报文静默超过宽限，即掉线）
  - **判定按测量步长做，不看滚动平均**：`MessageAgg` 的实测周期是 EMA，5 倍停顿会被抹平成 1.4 倍、要二十个样本才收敛；`Min/Max` 又是终身闭锁，一次抖动就永久定罪。所以周期用的是"这一步相对上一步的真实间隔"，只在确实带来了新帧时才判
  - **报告是历史不是状态灯**：某条判定一旦上榜就留在表里，只有持续违规才涨 `Count`，恢复干净也不会撤销——否则一个 3 倍停顿会在下一帧从眼前消失，而那正是你要查的东西。清空只有 **Clear** 按钮或重新开始一次测量（`reset` 与清空聚合同一批）
  - **阈值就地可改并随工程保存**：容差默认 ±10%、宽限默认 3 倍周期，窗口顶部直接拖，**松手才生效**（同生成器的规矩，绝不半路按新阈值重判）；两个数值存在工程文件的 `spec` 块里，旧工程没这一项则各取默认值。顶部四个勾选只按类型过滤显示、会话内有效，不进工程
  - **"缺席"只算"说过话之后掉线"**：仿真时总线上只有我们勾选的节点，若"声明为周期但从没出现"也算违规，一启动就会刷出整库其他节点的 missing——那是噪声，会淹掉真正的掉线。所以从没发过的报文保持沉默，发过然后停了几倍周期才报警。回放同理：日志开头就没有的报文不报，中途消失的报
  - **两条边界**：纯仿真总线上只有我们在发，而自己发的帧只判 **Unknown** 与 **Missing**——周期有生成器的 `DBC …ms` 小牌管着，DLC 则因为驱动高位信号时会故意把载荷拉长到超过声明值，判了必错；**Cycle / Dlc 的价值在回放与录制分析那边**。没挂 DBC 的总线完全沉默（没有数据库就没有观点），窗口顶部那行 `N periodic messages declared` 是数出来的：有些库写了 `BA_DEF_DEF_ "GenMsgCycleTime" 0`，那会让每条报文看起来都是事件触发、所有时间判定一起失声，这行数字就是唯一防线。判定以裸 ID 做键（与聚合和 DBC 一致），所以同值的标准帧与扩展帧会互相顶名，可能假报 Unknown / Dlc
...
- **多 Desktop**：类 CANoe 的多桌面——每个桌面各自记住五类观测窗口（Trace/Messages/Statistics/Graphics/Data）的开关与布局，以及全局面板（Generator/Network/Measurement Setup/Buses/ID Filter/Specification）的显示状态；底部桌面标签栏一键切换，`+` 新建空白桌面，右键标签可重命名、删除或调整顺序（Move to 选择目标位次，至少保留一个）；桌面列表与活动桌面随工程文件保存
- **工程文件（.rxproj）**：类 CANoe 的工程文件——总线数量/名称与各总线 DBC、每条总线勾选仿真的 DBC 节点、全部观测窗口及其过滤设置、已选信号、生成器配置、规格监视的容差与宽限、窗口布局/停靠全部打包在一个 JSON 工程文件里；规格监视的**违规行不进工程**——那是运行的观测结果而不是配置，重开工程应从空表开始；勾选的节点只记意图，打开工程不会因此自己发起流量（实际发不发仍由每条报文的 On 决定），旧工程没有这一项则按全部未勾选加载；File 菜单提供 New / Open / Recent Projects / Save / Save As；DBC 路径相对工程目录存储，工程文件夹可整体移动；已保存工程退出时自动覆盖，未保存（Untitled）工程仅在有修改时于退出或切换时弹框询问（未改动则静默通过）；New Project 创建完全空的工程（无 DBC、无观测窗口、无生成器条目）；上次打开的工程自动恢复（`roxy-can.meta.json` 记录）；运行中每 30 秒写一份崩溃缓存（`roxy-can.autosave.rxproj`，不动工程文件本身），异常退出后下次启动自动恢复，正常退出时删除；旧版 `roxy-can.json` 在首次启动时自动迁移。窗口可停靠、可拖动；HiDPI 由平台层按 framebuffer scale 自动换算；字体沿用 roxy-dbc 的方案——内嵌 Inconsolata（13px、像素对齐）并合并系统中文字体字形，按基线自然对齐，支持中文输入法（IME）
...
3. **View → Buses** 管理总线：改名、**Open...** 为单条总线加载 DBC、**+ Add bus** 新增、**x** 删除；**Open Log...** 只加载日志（ASC 或 BLF），回放由 **Play** 启动；`<<` `>>` 逐级变倍速，Stop 后的倍速下拉直接选择（0.5x/1x/2x/4x）；倍速下拉右侧的**时间轴可拖动定位**到任意时刻（暂停时拖完再 Play 从落点继续；日志播完后拖回去再按 Play 会原地续播，按 **Stop** 再 Play 才从头重放）；**回放进行中不能更换日志**——`Open Log...` 与 Recent Logs 会置灰，需先 **Stop**（Stop 后换日志会强制从头打开新文件，不会续播上一个）；总线挂了 DBC 之后，**View → Specification** 就是"流量与数据库吵嘴"的地方——仿真里把一个节点勾上去跑几秒再取消勾选，它负责的报文会以 **Missing** 上榜（掉线），回放真实日志则能看到 **Cycle / Dlc / Unknown** 具体是哪几条报文与声明不符，容差与宽限就在窗口顶部拖
