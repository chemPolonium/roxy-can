...
- 命令行可脱离界面运行与校验（CI 友好）：`--check-script` 编译节点脚本；`--project <p.rxproj> --duration <s> [--stats <csv>]` 无头仿真整个工程（生成器 + 脚本节点按保存的激活状态上线）；`--profile <名>` 叠加 `profiles/<名>.toml` 角色覆盖，同一工程在 simulation / bench / CI 间零修改切换，错配整份拒绝，非零退出报告失败
